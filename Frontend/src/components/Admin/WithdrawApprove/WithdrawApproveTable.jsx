...
function WithdrawApproveTable() {
    const [users, setUsers] = useState([]);

    useEffect(() => {
      fetch('/api/webapi/admin/withdraw-approve')
        .then((res) => {
          if (!res.ok) {
        ...
        {users.map((user, index) => (
          user.id ? (
            ...
            <tr key={index} style={{textAlign: "center"}}>
              ...
              <td >  <span
                      className={` btn-rounded p-1 text-light ${user.status === 1 ? 'btn-success' : 'btn-warning'}`}
...
export default WithdrawApproveTable
